...
{
    public class OsseousAshManagementRow : MonoBehaviour, IFrameworkControl
    {
        ...
        /*public static Dictionary<InputButtonTypes, Action> ButtonHandlers => new()
        {
            { InputButtonTypes.AcceptButton, Event.Helpers.Handle(OsseousAshManagement.instance.HandleDelete) },
        };
        public static Dictionary<string, Action> CommandHandlers => new()
        {
            { "CmdDelete", Event.Helpers.Handle(OsseousAshManagement.instance.HandleDelete) },
        };*/
        ...
        public void setData(FrameworkDataElement data)
        {
            ...
            {
                Invalid = true;
                //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(setData)} {nameof(Invalid)}: {Invalid} ({nameof(data)})");
                return;
            }

            Invalid = false;
            //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(setData)} {nameof(Invalid)}: {Invalid}");
            ...
            if (Instantiate(OsseousAshManagementRow.imageTinyFrame) is not ImageTinyFrame imageTinyFrame)
            {
                Invalid = true;
                //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(setData)} {nameof(Invalid)}: {Invalid} ({nameof(ImageTinyFrame)})");
                return;
            }
            ...
            if (TextSkins.IsNullOrEmpty())
            {
                ...
                {
                    Invalid = true;
                    //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(setData)} {nameof(Invalid)}: {Invalid} ({nameof(SaveManagement)}.{nameof(TextSkins)})");
                    return;
                }
            ...
            {
                Invalid = true;
                //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(setData)} {nameof(Invalid)}: {Invalid} ({nameof(TextSkins)})");
                return;
            }
        ...
        public void Update()
        {
            ...
            if (isActive != WasSelected)
            {
                ...
                if (TextSkins.IsNullOrEmpty())
                {
                    ...
                    {
                        Invalid = true;
                        //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(Update)} {nameof(Invalid)}: {Invalid} ({nameof(SaveManagement)}.{nameof(TextSkins)})");
                        return;
                    }
                    foreach (var textSkin in textSkins)
                    {
                ...
                {
                    Invalid = true;
                    //Utils.Log($"{1.Indent()}{nameof(OsseousAshManagementRow)}.{nameof(Update)} {nameof(Invalid)}: {Invalid} ({nameof(TextSkins)}2)");
                    return;
                }
